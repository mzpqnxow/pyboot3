...
Hacked by Adam Greene, for use in pyboot3 on ppc64le
"""
...
def get_makefile_filename():
    ...
    if python_build:
        makefile_path = os.path.join(_sys_home or project_base, "Makefile")
        if os.path.exists(makefile_path):
            return makefile_path
    lib_dir = get_python_lib(plat_specific=0, standard_lib=1)
    config_file = 'config-{}{}'.format(get_python_version(), build_flags)
    config_dir = os.path.join(lib_dir, config_file)
    # NOTE(AG): Added logic for compatibility on ppc64le
    if not os.path.exists(config_dir):
        config_dir_list = glob.glob(config_dir + '*')
        if config_dir_list and len(config_dir_list) == 1:
            config_dir = config_dir_list.pop()
            print(config_dir)
